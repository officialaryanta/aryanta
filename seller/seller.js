...
/* ===== Aryanta Android/WebView + Mobile Protection Patch =====
   1) Blocks long-press copy/open menu in mobile browsers/WebView.
   2) Auto-adds Razorpay webview_intent:true so UPI Intent can open from Android WebView.
   Keep this at the end of seller.js. */
(function aryantaMobileWebViewPatch(){
    if(window.__aryantaMobileWebViewPatchInstalled) return;
    window.__aryantaMobileWebViewPatchInstalled = true;

    function isEditable(el){
        if(!el) return false;
        if(el.closest && el.closest('input, textarea, select, option, [contenteditable="true"], .allow-copy, .allow-context-menu')) return true;
        return false;
    }

    function installNoCopyProtection(){
        try{
            document.documentElement.classList.add('aryanta-secure-ui');
            if(document.body) document.body.classList.add('aryanta-secure-ui');

            ['contextmenu','selectstart','dragstart'].forEach(function(evtName){
            ...
            ['copy','cut'].forEach(function(evtName){
                document.addEventListener(evtName, function(e){
                    if(!isEditable(e.target)){
                        e.preventDefault();
                        if(typeof showToast === 'function') showToast('Copy is disabled in Aryanta Seller App.', 'warning');
                        return false;
                    }
                }, {capture:true, passive:false});
            });
            ...
            document.addEventListener('touchstart', function(e){
                if(!isEditable(e.target) && e.target && e.target.tagName === 'IMG'){
                    e.target.setAttribute('draggable','false');
                }
            }, {capture:true, passive:true});
        }catch(e){ console.warn('Aryanta no-copy protection skipped:', e); }
    }

    function enhanceRazorpayOptions(options){
        options = options || {};
        try{
            options.webview_intent = true;
            options.retry = options.retry || {enabled:true, max_count:2};
            options.modal = options.modal || {};
            options.theme = options.theme || {color:'#0f172a'};

            // Keep all methods available, but make UPI reliably visible in WebView checkout.
            options.config = options.config || {};
            options.config.display = options.config.display || {};
            if(!options.config.display.blocks){
                options.config.display.blocks = {
                ...
                options.config.display.sequence = ['block.upi','block.card','block.netbanking','block.wallet'];
                options.config.display.preferences = {show_default_blocks:true};
            }
        }catch(e){}
        return options;
    }
    window.aryantaEnhanceRazorpayOptions = enhanceRazorpayOptions;

    function patchRazorpayConstructor(){
        try{
            if(!window.Razorpay || window.Razorpay.__aryantaWrapped) return !!(window.Razorpay && window.Razorpay.__aryantaWrapped);
            var NativeRazorpay = window.Razorpay;
            var WrappedRazorpay = function(options){
                return new NativeRazorpay(enhanceRazorpayOptions(options));
            };
            for(var k in NativeRazorpay){ try{ WrappedRazorpay[k] = NativeRazorpay[k]; }catch(e){} }
            WrappedRazorpay.prototype = NativeRazorpay.prototype;
            WrappedRazorpay.__aryantaWrapped = true;
            window.Razorpay = WrappedRazorpay;
            return true;
        }catch(e){ console.warn('Razorpay WebView patch skipped:', e); return false; }
    }

    installNoCopyProtection();
    if(document.readyState === 'loading'){
        document.addEventListener('DOMContentLoaded', installNoCopyProtection, {once:true});
    }

    var tries = 0;
    var timer = setInterval(function(){
        tries++;
        if(patchRazorpayConstructor() || tries > 80) clearInterval(timer);
    }, 250);
})();
