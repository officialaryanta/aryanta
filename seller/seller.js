...
function renderStatusScreen(title,msg,isSuspended=false,endTime=null){
    ...
    function normalizeSellerStatus(s){
    return String(s || "").toLowerCase().trim();
}

async function forceSellerOfflineNow(reason){
    try{
        ...
        await db.collection("sellers").doc(activeSeller.email).set({
            settings: activeSeller.settings,
            offline: true,
            offlineReason: reason || "Account restricted",
            offlineForcedAt: new Date().toISOString()
        }, { merge:true });

        try{ await enforceHiddenInventory(); }catch(e){}
    }catch(e){
        console.warn("Force offline skipped:", e);
    }
}

async function lockSellerPanelNow(statusData){
    if(!statusData) return;

    const status = normalizeSellerStatus(
        statusData.status || statusData.accountStatus || statusData.sellerStatus
    );

    if(status !== "blocked" && status !== "block" && status !== "suspended" && status !== "suspend"){
        return;
    }

    accountHardLocked = true;
    activeSeller = { ...(activeSeller || {}), ...statusData };
    ...
    document.querySelectorAll(".modal.show, .modal").forEach(m => {
    ...
    if(lo){
        ...
        lo.style.zIndex = "9999999";
    }

    if(status === "blocked" || status === "block"){
        await forceSellerOfflineNow("Account blocked by admin");

        renderStatusScreen(
            "Account Blocked",
            "Your seller account has been blocked by Admin. Your seller panel is locked immediately and your products are forced offline.",
            false
        );
    ...
    if(status === "suspended" || status === "suspend"){
        ...
        const suspendTime = new Date(suspendedAt).getTime();
        const unlockTime = suspendTime + (7 * 24 * 60 * 60 * 1000);

        renderStatusScreen(
            "Account Suspended",
            "Your seller account has been suspended by Admin. Your seller panel is locked immediately and your products are forced offline.",
            true,
            unlockTime
        );
...
function startSellerStatusRealtime(){
    try{
        if(!activeSeller || !activeSeller.email || !db) return;

        if(sellerStatusUnsubscribe){
            try{ sellerStatusUnsubscribe(); }catch(e){}
            sellerStatusUnsubscribe = null;
        }

        sellerStatusUnsubscribe = db.collection("sellers")
            .doc(activeSeller.email)
            .onSnapshot(async snap => {
                if(!snap.exists) return;

                const liveData = snap.data() || {};
                const status = normalizeSellerStatus(
                    liveData.status || liveData.accountStatus || liveData.sellerStatus
                );

                if(status === "blocked" || status === "block" || status === "suspended" || status === "suspend"){
                    await lockSellerPanelNow(liveData);
                }
            }, err => {
                console.warn("Seller status realtime listener error:", err);
            });
    }catch(e){
        console.warn("Seller status realtime could not start:", e);
    }
}
...
window.checkAdminPopups = async function() {
    ...
    try {
        const snap = await db.collection("seller_popups")
            .where("sellerEmail", "==", activeSeller.email)
            .get();
 for (let doc of snap.docs) {
            const pData = doc.data();
            
            if (pData.isRead !== true) { // If it is not marked as read yet
                const title = document.getElementById("adminPopupTitle");
                const msg = document.getElementById("adminPopupMessage");
                const modal = document.getElementById("adminPopupModal");
                
                if (title) title.innerText = pData.title || "Aryanta Notice";
                if (msg) msg.innerText = pData.message || pData.text || "";
                
                if (modal) {
                    modal.style.display = "flex";
                    setTimeout(() => modal.classList.add("show"), 10);
                }
               await db.collection("seller_popups").doc(doc.id).update({ 
                    isRead: true 
                });
...
window.showSection=function(section){
    if(accountHardLocked){
    return showToast("Account restricted. Seller panel is locked.", "error");
}
    const sb=document.getElementById('mobileSidebar');
...
async function initDashboard(){
    ...
    setRealLoader(5, "Checking secure seller session...");

   function setRealLoader(percent, msg){
    const loadPercent = document.getElementById("loadPercent");
    ...
    if(loaderMessage && msg) loaderMessage.innerText = msg;
}
...
setRealLoader(15, "Loading company settings...");
    ...
    try{
        ...
        setRealLoader(35, "Loading your products...");
...
setRealLoader(60, "Loading your orders...");
        ...
        setRealLoader(72, "Loading payments and fines...");
        ...
        setRealLoader(88, "Preparing dashboard...");
    }catch(e){}
   setRealLoader(95, "Opening seller panel...");
...
setRealLoader(100, "Seller panel ready.");
...
(function(){
   ...
   async function bootstrapInitialSellerData(){
    if(window.__ARYANTA_RAM.bootLoaded || window.__ARYANTA_RAM.booting) return;

    window.__ARYANTA_RAM.booting = true;

    setBootLoader('Loading seller dashboard.', 12);

    await fetchProductsCore();

    setBootLoader('Loading recent orders.', 45);

    await fetchOrdersCore();

    setBootLoader('Preparing dashboard.', 78);
    ...
    try{ renderBrandingPreviewsFinal(); }catch(e){}
...
(function(){
 ...
 window.__infoFileFilter = window.__infoFileFilter || "all";
window.__infoFilesCache = window.__infoFilesCache || null;
window.__infoFileTextCache = window.__infoFileTextCache || {};
window.__infoFileBlobCache = window.__infoFileBlobCache || {};
...
function infoExt(url){
    const clean = String(url || "").split("?")[0].split("#")[0].toLowerCase();
    const m = clean.match(/\.([a-z0-9]+)$/i);
    return m ? m[1] : "";
}

function infoGetItem(i){
    return (window.__infoFilesCache || [])[Number(i)] || null;
}

function infoIsImageExt(ext){
    return ["png","jpg","jpeg","webp","gif","svg","bmp"].includes(String(ext || "").toLowerCase());
}

function infoIsTextExt(ext){
    return ["txt","csv","json","html","htm","css","js","md","xml","log"].includes(String(ext || "").toLowerCase());
}

function infoIsOfficeExt(ext){
    return ["doc","docx","xls","xlsx","ppt","pptx"].includes(String(ext || "").toLowerCase());
}

window.setInfoFileFilter = function(type){
    window.__infoFileFilter = type || "all";
    renderInfoFilesFromCache();
};

window.downloadInfoFile = function(url, name){
    url = infoNormalizeUrl(url);
    if(!url) return showToast && showToast("File link missing.", "error");
...
window.downloadInfoFileByIndex = function(i){
    const f = infoGetItem(i);
    if(!f) return;
    downloadInfoFile(f.originalUrl || f.url, f.name);
};

window.openInfoImage = function(url){
    url = infoNormalizeUrl(url);

    const old = document.getElementById("infoImageFullModal");
    if(old) old.remove();

    const div = document.createElement("div");
    div.id = "infoImageFullModal";
    div.style.cssText = "position:fixed;inset:0;background:rgba(0,0,0,.92);z-index:999999;display:flex;align-items:center;justify-content:center;padding:18px;";
    div.innerHTML = `
        <button onclick="document.getElementById('infoImageFullModal').remove()"
            style="position:absolute;top:18px;right:18px;background:white;color:#111;border:0;border-radius:50%;width:42px;height:42px;font-size:22px;font-weight:900;cursor:pointer;">×</button>
        <img src="${infoEsc(url)}" style="max-width:96%;max-height:92vh;border-radius:16px;object-fit:contain;background:white;">
    `;
    document.body.appendChild(div);
};

window.openInfoFileByIndex = function(i){
    const f = infoGetItem(i);
    if(!f) return showToast && showToast("File not found.", "error");
...
window.openInfoFile = function(f){
    const url = f.ramUrl || f.url || f.originalUrl;
    const ext = f.ext || infoExt(url);
    ...
    const old = document.getElementById("infoFilePreviewModal");
    if(old) old.remove();

    const div = document.createElement("div");
    div.id = "infoFilePreviewModal";
    div.style.cssText = "position:fixed;inset:0;background:rgba(0,0,0,.76);z-index:999999;display:flex;align-items:center;justify-content:center;padding:18px;";

    div.innerHTML = `
        <div style="background:white;color:#111;width:min(1040px,96vw);max-height:92vh;border-radius:18px;overflow:hidden;box-shadow:0 30px 80px rgba(0,0,0,.35);">
            <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;padding:14px 18px;border-bottom:1px solid #e5e7eb;">
                <b style="word-break:break-word;">${infoEsc(f.name || "File Preview")}</b>
                <button onclick="document.getElementById('infoFilePreviewModal').remove()"
                    style="border:0;background:#111;color:white;border-radius:50%;width:34px;height:34px;font-size:18px;cursor:pointer;">×</button>
            </div>
            <div id="infoFilePreviewBody" style="padding:18px;overflow:auto;max-height:78vh;font-size:13px;line-height:1.6;"></div>
        </div>
    `;

    document.body.appendChild(div);

    const body = document.getElementById("infoFilePreviewBody");
    if(!body) return;

    if(infoIsTextExt(ext)){
        const text = window.__infoFileTextCache[f.originalUrl] || f.textContent || "";
        const pre = document.createElement("pre");
        pre.style.cssText = "white-space:pre-wrap;word-break:break-word;margin:0;font-family:Consolas,monospace;font-size:12px;line-height:1.55;";
        pre.textContent = text || "Text file loaded, but no readable text found.";
        body.innerHTML = "";
    ...
    if(ext === "pdf"){
        body.innerHTML = `
            <iframe src="${infoEsc(url)}" style="width:100%;height:72vh;border:0;border-radius:12px;background:#f1f5f9;"></iframe>
            <div style="display:flex;gap:8px;margin-top:10px;flex-wrap:wrap;">
                <button class="btn-prime" onclick="window.open('${infoEsc(f.originalUrl)}','_blank','noopener')">
                    <i class="fas fa-external-link-alt"></i> Open Original
                </button>
                <button class="btn-outline" onclick="downloadInfoFile('${infoEsc(f.originalUrl)}','${infoEsc(f.name)}')">
    ...
    if(infoIsOfficeExt(ext)){
        const viewer = "https://docs.google.com/gview?embedded=1&url=" + encodeURIComponent(f.originalUrl);
        body.innerHTML = `
            <iframe src="${infoEsc(viewer)}" style="width:100%;height:72vh;border:0;border-radius:12px;background:#f1f5f9;"></iframe>
            <div style="display:flex;gap:8px;margin-top:10px;flex-wrap:wrap;">
                <button class="btn-prime" onclick="window.open('${infoEsc(f.originalUrl)}','_blank','noopener')">
                    <i class="fas fa-external-link-alt"></i> Open Original
                </button>
            ...
            <p style="color:#64748b;margin-top:8px;font-weight:700;">DOC/DOCX/XLSX are binary files. They cannot be shown as raw text without a document reader.</p>
        `;
        return;
    }
    ...
    body.innerHTML = `
        <div style="text-align:center;padding:35px;">
            <i class="fas fa-file" style="font-size:55px;color:#64748b;margin-bottom:15px;"></i>
            <h3>Preview not supported</h3>
            <p style="color:#64748b;margin:10px 0 18px;">This file type cannot be safely previewed as text.</p>
            <button class="btn-prime" onclick="window.open('${infoEsc(f.originalUrl)}','_blank','noopener')">
                <i class="fas fa-external-link-alt"></i> Open File
            </button>
            <button class="btn-outline" style="margin-left:8px;" onclick="downloadInfoFile('${infoEsc(f.originalUrl)}','${infoEsc(f.name)}')">
                <i class="fas fa-download"></i> Download
            </button>
        </div>
    `;
...
async function preloadInfoFileToRam(f){
    if(!f || !f.originalUrl || f.kind === "link") return f;

    const ext = f.ext;

    try{
        if(infoIsTextExt(ext)){
            if(!window.__infoFileTextCache[f.originalUrl]){
                const res = await fetch(f.originalUrl, {cache:"force-cache"});
                if(res.ok){
                    window.__infoFileTextCache[f.originalUrl] = await res.text();
                }
            }
            f.textContent = window.__infoFileTextCache[f.originalUrl] || "";
            return f;
        }

        if(infoIsImageExt(ext) || ext === "pdf"){
            if(!window.__infoFileBlobCache[f.originalUrl]){
                const res = await fetch(f.originalUrl, {cache:"force-cache"});
                if(res.ok){
                    const blob = await res.blob();
                    window.__infoFileBlobCache[f.originalUrl] = URL.createObjectURL(blob);
                }
            }
            f.ramUrl = window.__infoFileBlobCache[f.originalUrl] || f.originalUrl;
            f.url = f.ramUrl;
            return f;
        }
    }catch(e){
...
window.renderInfoFilesFromCache = function(){
    const detail = $("infoDetailBox");
    if(!detail) return;

    let docs = window.__infoFilesCache || [];

    if(window.__infoFileFilter === "images") docs = docs.filter(f => f.kind === "image");
    if(window.__infoFileFilter === "files") docs = docs.filter(f => f.kind === "file");
    if(window.__infoFileFilter === "links") docs = docs.filter(f => f.kind === "link");

    let html = `
        <div class="info-detail-head-row">
            <h4>Company Files</h4>
            <button class="btn-outline" onclick="showInfoHome()"><i class="fas fa-arrow-left"></i> Back</button>
        </div>

        <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:18px;">
            <button class="cat-pill ${window.__infoFileFilter === "all" ? "active" : ""}" onclick="setInfoFileFilter('all')">
                <i class="fas fa-layer-group"></i> All
            </button>
            <button class="cat-pill ${window.__infoFileFilter === "images" ? "active" : ""}" onclick="setInfoFileFilter('images')">
                <i class="fas fa-image"></i> Images
            </button>
            <button class="cat-pill ${window.__infoFileFilter === "files" ? "active" : ""}" onclick="setInfoFileFilter('files')">
                <i class="fas fa-file"></i> Files
            </button>
            <button class="cat-pill ${window.__infoFileFilter === "links" ? "active" : ""}" onclick="setInfoFileFilter('links')">
                <i class="fas fa-link"></i> Links
            </button>
        </div>

        <div class="info-file-grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:14px;">
    `;

    if(!docs.length){
        html += `
            <div class="ary-empty-card" style="grid-column:1/-1;">
        ...
        docs.forEach(function(f){
            const realIndex = (window.__infoFilesCache || []).indexOf(f);

            html += `
                <div class="info-file-card" style="border:1px solid var(--border-color);border-radius:18px;padding:14px;background:var(--white);box-shadow:var(--shadow-sm);">
                    ${
                        f.kind === "image"
                        ? `<img src="${infoEsc(f.url)}" onclick="openInfoFileByIndex(${realIndex})" style="cursor:pointer;width:100%;height:160px;object-fit:cover;border-radius:14px;background:#f1f5f9;">`
                        : `<div class="info-file-icon" style="height:160px;border-radius:14px;background:#f1f5f9;display:flex;align-items:center;justify-content:center;font-size:44px;color:#64748b;"><i class="fas ${f.kind === "link" ? "fa-link" : "fa-file"}"></i></div>`
                    }

                    <h4 style="margin:10px 0;word-break:break-word;">${infoEsc(f.name)}</h4>

                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:10px;">
                        <button class="btn-prime" onclick="openInfoFileByIndex(${realIndex})">
                            <i class="fas fa-eye"></i> Open
                        </button>
                        ${
                            f.kind === "link"
                            ? `<button class="btn-outline" onclick="navigator.clipboard.writeText('${infoEsc(f.originalUrl)}'); showToast('Link copied','success');"><i class="fas fa-copy"></i> Copy</button>`
                            : `<button class="btn-outline" onclick="downloadInfoFileByIndex(${realIndex})"><i class="fas fa-download"></i> Download</button>`
                        }
                    </div>
                </div>
            `;
...
window.loadInfoFiles = async function(){
    ...
    if(window.__infoFilesCache){
        renderInfoFilesFromCache();
        return;
    }

    detail.innerHTML = '<div style="text-align:center;padding:40px;"><i class="fas fa-spinner fa-spin"></i> Loading all files into RAM...</div>';

    try{
        const collections = ["seller_files","admin_info_files","company_files","info_files","seller_info_files","aryanta_info_files"];
        const myEmail = String(activeSeller && activeSeller.email ? activeSeller.email : "").toLowerCase().trim();
        let rawRows = [];
        ...
        for(const col of collections){
            try{
                ...
                snap.forEach(function(doc){
                    ...
                    const target = String(d.target || d.audience || d.visibility || d.for || d.sellerEmail || d.email || "all").toLowerCase().trim();
                    const fileSeller = String(d.sellerEmail || d.email || d.seller || d.vendorEmail || "").toLowerCase().trim();

                    if(target !== "all" && target !== "seller" && target !== "sellers" && target !== myEmail && fileSeller !== myEmail) return;

                    rawRows.push({id:doc.id, collection:col, data:d});
                });
            }catch(e){
                console.warn("Skipped info collection:", col, e);
            }
        }
        ...
        rawRows.forEach(function(row){
            ...
            if(Array.isArray(d.files)) arr.push(...d.files);
            if(Array.isArray(d.attachments)) arr.push(...d.attachments);
            if(!arr.length) arr.push(d);

            arr.forEach(function(x, idx){
                const fileUrl = infoNormalizeUrl(x.fileUrl || x.downloadUrl || x.imageUrl || x.photoUrl || d.fileUrl || d.downloadUrl || d.imageUrl || d.photoUrl || "");
                const linkUrl = infoNormalizeUrl(x.link || x.url || x.actionUrl || x.buttonLink || d.link || d.actionUrl || d.buttonLink || "");

                const hasFileField = !!fileUrl;
                const finalUrl = fileUrl || linkUrl;
                if(!finalUrl) return;

                const name = x.fileName || x.name || x.title || d.fileName || d.name || d.title || d.message || (idx ? "Company Item " + (idx + 1) : "Company Item");
                const ext = infoExt(finalUrl) || String(x.type || x.fileType || d.type || d.fileType || "").toLowerCase().replace("image/","");

                const isImg = infoIsImageExt(ext) || String(x.type || d.type || "").toLowerCase().includes("image");
                const kind = !hasFileField && linkUrl ? "link" : (isImg ? "image" : "file");
        ...
        const seen = new Set();
        window.__infoFilesCache = items.filter(function(f){
            const key = f.kind + "|" + f.originalUrl;
            if(seen.has(key)) return false;
            seen.add(key);
            return true;
        });

        detail.innerHTML = '<div style="text-align:center;padding:40px;"><i class="fas fa-spinner fa-spin"></i> Reading files into RAM...</div>';

        await Promise.all(window.__infoFilesCache.map(preloadInfoFileToRam));
        ...
        console.error(e);
        detail.innerHTML = `
            <div class="ary-empty-card">
                <h3>Error loading files</h3>
                <p>${infoEsc(e.message || e)}</p>
            </div>
        `;
    }
};
...
(function(){
   ...
   // Admin support file preview + DB save.
function updateSupportUploadProgress(percent, msg, type){
    const preview = $('supportFilePreview');
    if(!preview) return;

    let box = $('supportUploadProgressBox');

    if(!box){
        box = document.createElement('div');
        box.id = 'supportUploadProgressBox';
        box.className = 'support-upload-progress-box';
    ...
    box.style.display = 'block';
    box.innerHTML = `
        <div class="support-upload-progress-head">
            <strong>${msg || 'Uploading...'}</strong>
            <span>${percent}%</span>
        </div>
        <div class="support-upload-bar">
            <div class="${type === 'success' ? 'success' : ''}" style="width:${percent}%"></div>
        </div>
    `;
}

function hideSupportUploadProgress(){
    const box = $('supportUploadProgressBox');
...
function readFileAsDataUrl(file, onProgress){
  ...
  async function buildSupportAttachments(){
    ...
    supportFilesCache.forEach(file => total += file.size || 1);
    if(!total) total = 1;

    updateSupportUploadProgress(0, 'Preparing upload...', '');
    ...
    for(const file of supportFilesCache){
        ...
        const nextProgress = function(loaded){
            const percent = ((completed + loaded) / total) * 100;
            updateSupportUploadProgress(percent, 'Uploading proofs...', '');
        };

        if(file.size <= 750000 && (completed + file.size) <= 950000){
            try{
                meta.dataUrl = await readFileAsDataUrl(file, nextProgress);
            }catch(e){
                meta.readError = true;
            }
        }else{
            ...
            updateSupportUploadProgress(((completed + file.size) / total) * 100, 'Saving file details...', '');
        }

        completed += file.size || 1;
        updateSupportUploadProgress((completed / total) * 100, 'Uploading proofs...', '');

        out.push(meta);
    }

    updateSupportUploadProgress(100, 'Upload complete. Saving ticket...', 'success');
    return out;
}
    window.submitSupportTicket = async function(){
        ...
        try{
            updateSupportUploadProgress(0, 'Starting upload...', '');
            ...
            updateSupportUploadProgress(100, 'Upload success!', 'success');

setTimeout(() => {
    hideSupportUploadProgress();
}, 1500);
    ...
    function ensureAppLinkModal(){
        ...
        m.innerHTML = `<div class="modal-content ary-app-link-card">
            ...
            <div style="display:flex; gap:10px; justify-content:center; flex-wrap:wrap; margin-top:16px;">
                ...
                <button class="btn-prime" onclick="window.open('${PLAY_STORE_URL}','_blank','noopener')"></button>
            </div>
        </div>`;
        document.body.appendChild(m);
        return m;
    }
   window.downloadAryantaPlayStoreApp = function() {
        if(typeof showToast === 'function') {
            showToast('Downloading Aryanta App...', 'info');
        }
        const downloadLink = document.createElement('a');
        downloadLink.href = 'https://aryanta.in/app/app-debug.apk';
        downloadLink.target = '_blank';
        downloadLink.download = 'Aryanta_Seller_App'; 
...
(function(){
    ...
    function improveSettingsInstallCards(){
        ...
        document.querySelectorAll('button[onclick="downloadAryantaPlayStoreApp()"]').forEach(btn => {
            if(!btn.classList.contains('app-install-btn')){ btn.className = 'app-install-btn app-install-btn-store'; btn.removeAttribute('style'); btn.innerHTML = '</i><span>Download the App</span>'; }
        });
    }
    ...
    function ensureAppStatusModal(){
        ...
        m.innerHTML = `<div class="modal-content" style="max-width:440px;text-align:center;">
            ...
            <div style="display:flex;gap:10px;justify-content:center;flex-wrap:wrap;margin-top:18px;">
                ${PLAY_STORE_LINK ? `<button class="btn-prime" onclick="window.open('${PLAY_STORE_LINK}','_blank','noopener')"></i> Open Play Store</button>` : ''}
...
(function(){
   ...
   function smallProductCard(m, type){
    ...
    const action = planRank() >= 2
        ? `<button class="btn-outline analytics-open-btn" onclick="openInfoProductAnalytics('${js(m.id)}')"><i class="fas fa-chart-line"></i> Open Graph</button>`
        : '';
    ...
    return `<article class="analytics-product-mini ${cls}">
        ...
        <div>
            <b>${esc(m.name)}</b>
            <span>Orders: ${m.orders} • Sold: ${m.qty} • Cancel: ${m.cancelRate}%</span>
        </div>
        ${action}
    </article>`;
}
...
function ensureAnalyticsModal(){
    ...
    m.innerHTML = `<div class="modal-content" style="width:min(1080px,95vw);max-height:90vh;overflow:auto;">
...
window.closeInfoAnalyticsModal = function(){
    ...
    [
        '__INFO_PRODUCT_ORDER_CHART__',
        '__INFO_PRODUCT_CANCEL_CHART__',
        '__INFO_PRODUCT_PIE_CHART__',
        '__INFO_PRODUCT_HEALTH_CHART__',
        '__INFO_ULTRA_STORE_HEALTH_CHART__',
        '__INFO_ULTRA_STORE_TREND_CHART__',
        '__INFO_ULTRA_STORE_PIE_CHART__'
    ].forEach(k=>{
        try{
            if(window[k]){
                window[k].destroy();
...
function dailyLabelsFromMetric(m){
    const daily = m.daily || {};
    let labels = Object.keys(daily);
    if(!labels.length) labels = ['No Data'];
    return labels.slice(-14);
}

function drawProductGraphs(m){
    if(typeof Chart === 'undefined') return;

    const labels = dailyLabelsFromMetric(m);
    const daily = m.daily || {};

    try{ if(window.__INFO_PRODUCT_ORDER_CHART__) window.__INFO_PRODUCT_ORDER_CHART__.destroy(); }catch(e){}
    try{ if(window.__INFO_PRODUCT_CANCEL_CHART__) window.__INFO_PRODUCT_CANCEL_CHART__.destroy(); }catch(e){}
    try{ if(window.__INFO_PRODUCT_PIE_CHART__) window.__INFO_PRODUCT_PIE_CHART__.destroy(); }catch(e){}
    try{ if(window.__INFO_PRODUCT_HEALTH_CHART__) window.__INFO_PRODUCT_HEALTH_CHART__.destroy(); }catch(e){}

    const orderCanvas = $('infoProductOrderChartFinal');
    const cancelCanvas = $('infoProductCancelChartFinal');
    const pieCanvas = $('infoProductPieChartFinal');
    const healthCanvas = $('infoProductHealthChartFinal');

    if(orderCanvas){
        window.__INFO_PRODUCT_ORDER_CHART__ = new Chart(orderCanvas,{
            type:'line',
            data:{
                labels,
                datasets:[{
                    label:'Orders',
                    data:labels.map(d => daily[d] ? daily[d].orders : 0),
                    tension:.35,
                    fill:false
                }]
            },
    ...
    if(cancelCanvas){
        window.__INFO_PRODUCT_CANCEL_CHART__ = new Chart(cancelCanvas,{
            type:'bar',
            data:{
                labels,
                datasets:[
                    {
                        label:'Cancels',
                        data:labels.map(d => daily[d] ? daily[d].cancels : 0)
                    },
                    {
                        label:'Returns',
                        data:labels.map(d => daily[d] ? daily[d].returns : 0)
                    }
                ]
            },
            options:{
    ...
    if(pieCanvas){
        window.__INFO_PRODUCT_PIE_CHART__ = new Chart(pieCanvas,{
            type:'doughnut',
            data:{
                labels:['Delivered','Cancelled','Returned','Other'],
                datasets:[{
                    data:[
                        m.statusCounts.delivered || 0,
                        m.statusCounts.cancelled || 0,
                        m.statusCounts.returned || 0,
                        m.statusCounts.other || 0
                    ]
                }]
            },
            options:{
    ...
    if(healthCanvas){
        const risk = Math.min(100, Math.round((m.cancelRate || 0) + (m.returnRate || 0)));
        window.__INFO_PRODUCT_HEALTH_CHART__ = new Chart(healthCanvas,{
            type:'bar',
            data:{
                labels:['Health Score','Cancel Risk','Return Risk','Stock'],
                datasets:[{
                    label:'Deep Product Score',
                    data:[
                        m.health || 0,
                        m.cancelRate || 0,
                        m.returnRate || 0,
                        Math.min(100, Number(m.stock || 0))
                    ]
                }]
            },
            options:{
...
window.openInfoProductAnalytics = function(id){
    ...
    if(rank < 2){
        return toast('Pro plan required for product graph.','info');
    }
    ...
    const ultraExtra = rank >= 3 ? `
        <div class="ultra-chart-grid-final product-ultra-extra-grid">
            <div>
                <h4><i class="fas fa-heart-pulse"></i> Deep Product Health Graph</h4>
                <canvas id="infoProductHealthChartFinal"></canvas>
            </div>
        </div>
        <p class="analytics-note-final">
            <i class="fas fa-gem"></i> Ultra insight: this graph compares health score, cancel risk, return risk and stock pressure for deeper product decision.
        </p>
    ` : '';

    body.innerHTML = `
        <div class="product-analytics-slip-final">
            <div class="product-analytics-head-final">
                <img src="${esc(m.img || 'https://via.placeholder.com/120')}" onerror="this.src='https://via.placeholder.com/120'">
                <div>
                    <span>${rank >= 3 ? 'Ultra Product Analytics' : 'Pro Product Analytics'}</span>
                    <h2>${esc(m.name)}</h2>
                    <p>Product wise order graph, cancel graph and order status pie chart.</p>
                </div>
            </div>

            <div class="analytics-stat-grid-final">
                <div><span>Total Orders</span><strong>${m.orders}</strong></div>
                <div><span>Total Sold Qty</span><strong>${m.qty}</strong></div>
            ...
            <div class="ultra-chart-grid-final">
                <div>
                    <h4><i class="fas fa-chart-line"></i> Product Order Graph</h4>
                    <canvas id="infoProductOrderChartFinal"></canvas>
                </div>
                <div>
                    <h4><i class="fas fa-ban"></i> Product Cancel / Return Graph</h4>
                    <canvas id="infoProductCancelChartFinal"></canvas>
                </div>
                <div>
                    <h4><i class="fas fa-chart-pie"></i> Product Status Pie Chart</h4>
                    <canvas id="infoProductPieChartFinal"></canvas>
                </div>
            </div>
    ...
    setTimeout(()=>drawProductGraphs(m),80);
};

window.openUltraInfoGraph = function(){
    if(planRank() < 3){
        return toast('Ultra plan required for store health graph.','info');
    }

    const rows = metrics(0);
    if(!rows.length) return toast('No store analytics data found yet.','info');

    const modal = ensureAnalyticsModal();
    const body = $('infoAnalyticsModalBody');

    const totalOrders = rows.reduce((a,m)=>a + Number(m.orders || 0),0);
    const totalCancels = rows.reduce((a,m)=>a + Number(m.cancels || 0),0);
    const totalReturns = rows.reduce((a,m)=>a + Number(m.returns || 0),0);
    const totalSold = rows.reduce((a,m)=>a + Number(m.qty || 0),0);
    const totalRevenue = rows.reduce((a,m)=>a + Number(m.revenue || 0),0);
    const cancelRate = totalOrders ? Math.round((totalCancels / totalOrders) * 100) : 0;
    const orderRate = Math.round((totalOrders / Math.max(1,new Date().getDate())) * 10) / 10;

    body.innerHTML = `
        <div class="ultra-head-final">
            <i class="fas fa-gem"></i>
            <div>
                <span>Ultra Store Health</span>
                <h2>Store Analytics Graph</h2>
                <p>This popup shows store health only.</p>
            </div>
        </div>

        <div class="analytics-stat-grid-final">
            <div><span>Total Orders</span><strong>${totalOrders}</strong></div>
            <div><span>Total Cancels</span><strong>${totalCancels}</strong></div>
            <div><span>Total Cancel Rate</span><strong>${cancelRate}%</strong></div>
            <div><span>Order Rate</span><strong>${orderRate}/day</strong></div>
            <div><span>Total Sold Qty</span><strong>${totalSold}</strong></div>
            <div><span>Total Revenue</span><strong>${money(totalRevenue)}</strong></div>
        </div>

        <div class="ultra-chart-grid-final">
            <div>
                <h4><i class="fas fa-heart-pulse"></i> Store Health: Order Rate vs Cancel Rate</h4>
                <canvas id="ultraStoreHealthGraphFinal"></canvas>
            </div>
            <div>
                <h4><i class="fas fa-chart-line"></i> Store Orders / Cancels / Returns</h4>
                <canvas id="ultraStoreTrendGraphFinal"></canvas>
            </div>
            <div>
                <h4><i class="fas fa-chart-pie"></i> Store Status Mix</h4>
                <canvas id="ultraStorePieGraphFinal"></canvas>
            </div>
        </div>
    `;
    ...
    setTimeout(()=>{
        ...
        try{ if(window.__INFO_ULTRA_STORE_HEALTH_CHART__) window.__INFO_ULTRA_STORE_HEALTH_CHART__.destroy(); }catch(e){}
        try{ if(window.__INFO_ULTRA_STORE_TREND_CHART__) window.__INFO_ULTRA_STORE_TREND_CHART__.destroy(); }catch(e){}
        try{ if(window.__INFO_ULTRA_STORE_PIE_CHART__) window.__INFO_ULTRA_STORE_PIE_CHART__.destroy(); }catch(e){}

        const health = $('ultraStoreHealthGraphFinal');
        const trend = $('ultraStoreTrendGraphFinal');
        const pie = $('ultraStorePieGraphFinal');

        const topRows = rows.slice().sort((a,b)=>b.orders-a.orders).slice(0,8);
        const labels = topRows.map(x => x.name.length > 14 ? x.name.slice(0,14)+'...' : x.name);

        if(health){
            window.__INFO_ULTRA_STORE_HEALTH_CHART__ = new Chart(health,{
                type:'bar',
                data:{
                    labels:['Total Orders','Total Cancels','Cancel Rate %','Order Rate / Day'],
                    datasets:[{
                        label:'Store Health',
                        data:[totalOrders,totalCancels,cancelRate,orderRate]
                    }]
                },
                options:{
                    responsive:true,
        ...
        if(trend){
            window.__INFO_ULTRA_STORE_TREND_CHART__ = new Chart(trend,{
                type:'line',
                data:{
                    labels,
                    datasets:[
                        {label:'Orders',data:topRows.map(x=>x.orders),tension:.35},
                        {label:'Cancels',data:topRows.map(x=>x.cancels),tension:.35},
                        {label:'Returns',data:topRows.map(x=>x.returns),tension:.35}
                    ]
                },
                options:{
                    responsive:true,
        ...
        if(pie){
            const statusTotals = rows.reduce((a,m)=>{
                a.delivered += Number(m.statusCounts.delivered || 0);
                a.cancelled += Number(m.statusCounts.cancelled || 0);
                a.returned += Number(m.statusCounts.returned || 0);
                a.other += Number(m.statusCounts.other || 0);
                return a;
            },{delivered:0,cancelled:0,returned:0,other:0});

            window.__INFO_ULTRA_STORE_PIE_CHART__ = new Chart(pie,{
                type:'doughnut',
                data:{
                    labels:['Delivered','Cancelled','Returned','Other'],
                    datasets:[{
...
window.loadInfoAnalytics = function(){
    ...
    if(rank === 0){
        detail.innerHTML = `
            <div class="info-detail-head-row">
                <h4>Info Analytics</h4>
                <button class="btn-outline" onclick="showInfoHome()"><i class="fas fa-arrow-left"></i> Back</button>
            </div>

            <div class="perf-locked-v5">
                <i class="fas fa-lock"></i>
                <div>
                    <h3>Analytics locked</h3>
                    <p>Growth unlocks top 3 selling and losing products. Pro unlocks product graphs. Ultra unlocks store health and deep product graphs.</p>
                </div>
                <button class="btn-prime" onclick="showSection('subscription')">
    ...
    const rows = metrics(0);
    const top = topSelling(rows);
    const loss = topLosing(rows);

    if(!window.__infoAnalyticsView) window.__infoAnalyticsView = 'top';
    if(rank < 2) window.__infoAnalyticsView = 'top';

    const view = window.__infoAnalyticsView;

    const ultraBtn = rank >= 3 ? `
        <button class="btn-prime ultra-open-btn-final" onclick="openUltraInfoGraph()">
            <i class="fas fa-gem"></i> Open Ultra Graph
        </button>
    ` : '';

    const toggleBtns = rank >= 2 ? `
        <div class="analytics-toggle-final" style="display:flex;gap:10px;flex-wrap:wrap;margin:0 0 16px;">
            <button class="cat-pill ${view === 'top' ? 'active' : ''}" onclick="window.__infoAnalyticsView='top'; loadInfoAnalytics();">
                <i class="fas fa-ranking-star"></i> Top View
            </button>
            <button class="cat-pill ${view === 'all' ? 'active' : ''}" onclick="window.__infoAnalyticsView='all'; loadInfoAnalytics();">
                <i class="fas fa-layer-group"></i> See All Products
            </button>
            ${ultraBtn}
        </div>
    ` : '';
    ...
    const note = rank >= 3
        ? `<p class="analytics-note-final"><i class="fas fa-gem"></i> Ultra: Open Ultra Graph shows store health only. Click any product for deeper product graphs.</p>`
        : rank >= 2
            ? `<p class="analytics-note-final"><i class="fas fa-mouse-pointer"></i> Pro: click any product to view product order graph, cancel graph and status pie chart.</p>`
            : `<p class="analytics-note-final"><i class="fas fa-seedling"></i> Growth: showing only top 3 selling and top 3 losing products.</p>`;

    let mainContent = '';

    if(view === 'all' && rank >= 2){
        mainContent = `
            <div class="analytics-two-col-final" style="grid-template-columns:1fr;">
                <section>
                    <h4><i class="fas fa-layer-group"></i> All Products Analytics</h4>
                    <div class="perf-grid-v5">
                        ${
                            rows.map((m, i) => smallProductCard(m, i < 3 ? 'sell' : 'loss')).join('') ||
                            '<div class="ary-empty-card"><i class="fas fa-box-open"></i><h3>No products found</h3></div>'
                        }
                    </div>
                </section>
            </div>
        ...
        mainContent = `
            <div class="analytics-two-col-final">
                <section>
                    <h4><i class="fas fa-fire"></i> Top 3 Selling Products</h4>
                    ${
                        top.map(m => smallProductCard(m,'sell')).join('') ||
                        '<div class="ary-empty-card"><i class="fas fa-box-open"></i><h3>No selling data yet</h3></div>'
                    }
                </section>

                <section>
                    <h4><i class="fas fa-triangle-exclamation"></i> Losing Products</h4>
                    ${
                        loss.map(m => smallProductCard(m,'loss')).join('') ||
                        '<div class="ary-empty-card"><i class="fas fa-chart-simple"></i><h3>No losing data yet</h3></div>'
                    }
                </section>
            </div>
        `;
    ...
    detail.innerHTML = `
        ...
        <div class="analytics-hero-final">
            <div>
                <span>${esc(currentPlan())} Plan</span>
                <h3><i class="fas fa-chart-line"></i> Product Analytics</h3>
                <p>Product list stays in panel. Popup only shows graphs.</p>
            </div>
        </div>
...
window.addDownloadAppButton = function() {}; function addDownloadAppButton() {}
// This empty code stops the "addDownloadAppButton is not defined" error from happening ever again
window.addDownloadAppButton = function() {}; 
function addDownloadAppButton() {}

// This is the new popup and loading screen code
window.downloadAryantaPlayStoreApp = function() {
    
    // 1. Reset the popup so it starts at 0% every time
    const progressSection = document.getElementById('downloadProgressSection');
    const stepsSection = document.getElementById('downloadStepsSection');
    const progressBar = document.getElementById('downloadProgressBar');
    const percentText = document.getElementById('downloadPercentage');
    ...
    // 3. Start counting from 0 to 100
    let progress = 0;
    const interval = setInterval(() => {
        progress += Math.floor(Math.random() * 15) + 5; // Adds a random amount to look real
        ...
        if (progress >= 100) {
            ...
            // 4. Wait half a second, then show the steps and download the file
            setTimeout(() => {
                if(progressSection) progressSection.style.display = 'none';
                if(stepsSection) stepsSection.style.display = 'block';
                
                // Actually download the app file in the background
                const downloadLink = document.createElement('a');
                downloadLink.href = 'https://aryanta.in/app/app-debug.apk'; 
                downloadLink.target = '_blank';
                downloadLink.download = 'Aryanta_Seller_App.apk'; 
                downloadLink.style.display = 'none';
                document.body.appendChild(downloadLink);
                downloadLink.click();
    ...
    }, 400); // Changes the number every 0.4 seconds
};
const originalRenderNotifs = window.renderNotificationsAryanta;
window.renderNotificationsAryanta = function() {
    const seen = new Set();
    const deduplicate = (arr) => arr.filter(n => {
    ...
    if(window.__ARYANTA_RAM && Array.isArray(window.__ARYANTA_RAM.notifications)) {
        window.__ARYANTA_RAM.notifications = deduplicate(window.__ARYANTA_RAM.notifications);
    }
    if(typeof adminNotifications !== 'undefined' && Array.isArray(adminNotifications)) {
        adminNotifications = deduplicate(adminNotifications);
    }

    if(originalRenderNotifs) originalRenderNotifs.apply(this, arguments);
};

window.aryOpenNotification = window.openFullNotifFinal = window.openFullNotif = function(id) {
    let rows = [];
    if(window.__ARYANTA_RAM && Array.isArray(window.__ARYANTA_RAM.notifications)) rows = window.__ARYANTA_RAM.notifications;
    else if(typeof adminNotifications !== 'undefined') rows = adminNotifications;

    const n = rows.find(x => String(x.id) === String(id) || String(x.__readKey) === String(id));
    if(!n) return;

    const linkRaw = n.link || n.url || n.actionLink || "";
    const link = linkRaw ? (linkRaw.startsWith("http") ? linkRaw : "https://" + linkRaw) : "";
    const cont = document.getElementById("notifDetailContent");
    const mod = document.getElementById("notificationDetailModal");
    ...
    if(cont){
        cont.innerHTML = `<div class="notif-detail-v5">
            <div class="notif-detail-head">
                <div class="notif-icon-v5 big"><i class="fas fa-check"></i></div>
                <div>
                    <h3>${n.title || n.heading || "Aryanta Notice"}</h3>
                    <small>${new Date(n.time || n.timestamp || Date.now()).toLocaleDateString()}</small>
                </div>
            </div>
            <div class="notif-detail-message">${n.text || n.message || n.body || "No message"}</div>
            ${link ? `<a href="${link}" target="_blank" rel="noopener" class="btn-prime"><i class="fas fa-link"></i> Open Attached Link</a>` : ""}
        </div>`;
    }
    ...
    n.read = true;
    if(window.__ARYANTA_RAM) {
        window.__ARYANTA_RAM.notificationReadMap = window.__ARYANTA_RAM.notificationReadMap || {};
        window.__ARYANTA_RAM.notificationReadMap[id] = true;
        if(n.__readKey) window.__ARYANTA_RAM.notificationReadMap[n.__readKey] = true;
    }

    setTimeout(() => {
        window.renderNotificationsAryanta();
        try {
            const email = (window.activeSeller || {}).email;
            if(typeof db !== 'undefined' && db && email) {
                db.collection("sellers").doc(email).set({
                    notificationReadMap: window.__ARYANTA_RAM.notificationReadMap,
                    updatedAt: new Date().toISOString()
                }, {merge: true});
            }
        } catch(e) {}
    ...
    const interval = setInterval(() => {
        ...
        if (progress >= 100) {
            ...
            setTimeout(() => {
                ...
                const downloadLink = document.createElement('a');
                downloadLink.href = 'https://aryanta.in/app'; 
                downloadLink.target = '_blank';
                downloadLink.download = 'Aryanta_Seller_App.apk'; 
                downloadLink.style.display = 'none';
                document.body.appendChild(downloadLink);
...
window.checkAdminPopups = async function() {
    if(!activeSeller || !db) return;
    try {
        const snap = await db.collection("seller_popups").where("sellerEmail", "==", activeSeller.email).get();
        for (let doc of snap.docs) {
            const pData = doc.data();
            if (pData.isRead !== true) {
                const title = document.getElementById("adminPopupTitle");
                const msg = document.getElementById("adminPopupMessage");
                const modal = document.getElementById("adminPopupModal");
                if (title) title.innerText = pData.title || "Aryanta Notice";
                if (msg) msg.innerHTML = pData.message || pData.text || "";
                if (modal) { modal.style.display = "flex"; setTimeout(() => modal.classList.add("show"), 10); }
                await db.collection("seller_popups").doc(doc.id).update({ isRead: true });
                break;
            }
        }
    } catch(error) {}
...
window.__oldRenderNotifsBackup = window.__oldRenderNotifsBackup || window.renderNotificationsAryanta;
window.renderNotificationsAryanta = function() {
    const seen = new Set();
    const deduplicate = (arr) => arr.filter(n => {
        const id = n.id || n.title;
    ...
    if(window.__oldRenderNotifsBackup) window.__oldRenderNotifsBackup.apply(this, arguments);
};
...
window.aryOpenNotification = window.openFullNotifFinal = window.openFullNotif = function(id) {
    ...
    if(cont){
        let linkHtml = "";
        if (link) {
            linkHtml = `<div style="margin-top: 20px; padding: 15px; background: var(--surface-2); border-radius: 12px; border: 1px solid var(--border-color); text-align: center;">
                <p style="margin-bottom: 10px; font-weight: 800; font-size: 13px; color: var(--text-light);">Admin attached a link to this notice:</p>
                <a href="${link}" target="_blank" rel="noopener" class="btn-prime w-100" style="text-decoration: none;">
                    <i class="fas fa-external-link-alt"></i> Open Attached Link
                </a>
            </div>`;
        }
...
/* ===== FIX INFO > MY B2B ORDERS CLEAN VIEW ===== */
function aryB2BEsc(v){
    return String(v == null ? "" : v).replace(/[&<>"']/g, function(c){
        return {"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c];
    });
}

function aryB2BMoney(v){
    const n = Number(v || 0);
    return "₹" + n.toLocaleString("en-IN");
}

function aryB2BDate(v){
    try{
...
function aryB2BAddDays(d, days){
    const x = new Date(d);
    x.setDate(x.getDate() + days);
    return x;
}

function aryB2BStatusClass(status){
    status = String(status || "").toLowerCase();
    if(status.includes("cancel")) return "b2b-status-cancel";
    if(status.includes("deliver")) return "b2b-status-success";
    if(status.includes("paid")) return "b2b-status-paid";
    return "b2b-status-pending";
}

window.toggleB2bOrderDetail = function(idx){
    const el = document.getElementById("b2bOrderDetail_" + idx);
    if(!el) return;
    el.style.display = el.style.display === "none" ? "block" : "none";
};

window.loadInfoB2bOrders = async function(){
    const home = document.getElementById("infoHomeGrid");
    const detail = document.getElementById("infoDetailBox");
    ...
    if(document.getElementById("infoBackBtn")) document.getElementById("infoBackBtn").style.display = "inline-flex";
    if(document.getElementById("infoPanelTitle")) document.getElementById("infoPanelTitle").innerHTML = '<i class="fas fa-box-open"></i> My B2B Orders';
    if(document.getElementById("infoPanelSub")) document.getElementById("infoPanelSub").innerText = "Click any order to view clean details and expected delivery date.";

    detail.innerHTML = `<div class="ary-empty-card"><i class="fas fa-spinner fa-spin"></i><h3>Loading B2B Orders...</h3></div>`;

    try{
        const email = String((activeSeller && activeSeller.email) || "").toLowerCase().trim();
        const collections = ["b2b_orders","b2bOrders","seller_b2b_orders"];
        let rows = [];
        ...
        for(const col of collections){
            try{
                const snap = await db.collection(col).where("sellerEmail","==",email).get();
                snap.forEach(doc => rows.push({ id:doc.id, source:col, ...doc.data() }));
            }catch(e){}
        }
        ...
        rows.sort((a,b) => aryB2BDate(b.date || b.createdAt || b.timestamp || b.orderDate) - aryB2BDate(a.date || a.createdAt || a.timestamp || a.orderDate));

        if(!rows.length){
            detail.innerHTML = `
                <div class="info-detail-head-row">
                    <h4>B2B Order History</h4>
                </div>
                <div class="ary-empty-card">
                    <i class="fas fa-box-open"></i>
                    <h3>No B2B orders found</h3>
                    <p>Your seller B2B purchases will appear here after you place an order.</p>
                </div>
            `;
            return;
        }

        const html = rows.map((o,idx)=>{
            const date = aryB2BDate(o.date || o.createdAt || o.timestamp || o.orderDate);
            const eta = aryB2BAddDays(date, 7);

            const item = o.productName || o.itemName || o.name || (Array.isArray(o.items) && o.items[0] && (o.items[0].name || o.items[0].title)) || "B2B Item";
            const qty = o.qty || o.quantity || (Array.isArray(o.items) && o.items[0] && (o.items[0].qty || o.items[0].quantity)) || 1;
            const status = o.status || "Request Sent";
            const amount = o.totalPrice || o.totalAmount || o.amount || o.price || 0;
            const img = o.productImage || o.image || o.img || "";
            const address = [o.address, o.city, o.state, o.pincode].filter(Boolean).join(", ");

            return `
                <div class="b2b-order-card-clean" onclick="toggleB2bOrderDetail(${idx})">
                    <div class="b2b-order-left-clean">
                        ${img ? `<img src="${aryB2BEsc(img)}" onerror="this.style.display='none'">` : `<div class="b2b-no-img"><i class="fas fa-box"></i></div>`}
                        <div>
                            <strong>${aryB2BEsc(item)}</strong>
                            <span>${date.toLocaleDateString("en-IN")} • Qty ${aryB2BEsc(qty)}</span>
                        </div>
                    </div>
                    <div class="b2b-order-right-clean">
                        <b>${aryB2BMoney(amount)}</b>
                        <em class="${aryB2BStatusClass(status)}">${aryB2BEsc(status)}</em>
                    </div>
                </div>

                <div class="b2b-order-detail-clean" id="b2bOrderDetail_${idx}" style="display:none;">
                    <div class="b2b-eta-clean">
                        <i class="fas fa-truck"></i>
                        Expected to be delivered by <b>${eta.toLocaleDateString("en-IN")}</b>
                    </div>

                    <div class="b2b-detail-grid-clean">
                        <div><span>Order ID</span><strong>${aryB2BEsc(o.id || "N/A")}</strong></div>
                        <div><span>Product</span><strong>${aryB2BEsc(item)}</strong></div>
                        <div><span>Quantity</span><strong>${aryB2BEsc(qty)}</strong></div>
                        <div><span>Total Amount</span><strong>${aryB2BMoney(amount)}</strong></div>
                        <div><span>Status</span><strong>${aryB2BEsc(status)}</strong></div>
                        <div><span>Payment</span><strong>${aryB2BEsc(o.paymentMethod || o.payment || "Online")}</strong></div>
                        <div><span>Phone</span><strong>${aryB2BEsc(o.sellerPhone || o.phone || "N/A")}</strong></div>
                        <div><span>Seller Email</span><strong>${aryB2BEsc(o.sellerEmail || email || "N/A")}</strong></div>
                    </div>

                    ${address ? `
                        <div class="b2b-address-clean">
                            <span>Delivery Address</span>
                            <strong>${aryB2BEsc(address)}</strong>
                        </div>
                    ` : ""}
                </div>
            `;
        ...
        detail.innerHTML = `
            <div class="ary-empty-card">
                ...
                <h3>Error loading B2B orders</h3>
                <p>${aryB2BEsc(e.message || e)}</p>
            </div>
        `;
    }
};
/* ===== Support + Q&A read count + mobile Q&A card fix ===== */
(function(){
    const $ = id => document.getElementById(id);
    ...
    function sellerMail(){
        try{
            return String((activeSeller && activeSeller.email) || '').toLowerCase().trim();
        }catch(e){
            return '';
        }
    }
    ...
    function ensureNavBadge(labelText, id){
        let b = $(id);
        if(b) return b;

        const nav = Array.from(document.querySelectorAll('.nav-item')).find(n =>
            n.textContent.toLowerCase().includes(labelText.toLowerCase())
        );
    ...
    function setBadge(labelText, id, count){
        const b = ensureNavBadge(labelText, id);
        if(!b) return;
    ...
    function isTicketResolved(t){
        const s = low(t && t.status);
        return ['resolved','complete','completed','closed'].includes(s);
    }

    function ticketNeedsSellerRead(t){
        if(!t || isTicketResolved(t)) return false;

        const s = low(t.status);
        const isAdminReply =
            s === 'waiting for user' ||
            s.includes('requires') ||
            s.includes('reply');
        ...
        if(t.unreadForSeller === false) return false;

        const adminTime = timeMs(t.adminRepliedAt || t.adminReplyAt || t.lastAdminReplyAt || t.updatedAt || t.timestamp);
        const sellerReadTime = timeMs(t.sellerReadAt || t.readAtBySeller);

        if((t.readBySeller === true || t.sellerRead === true) && (!adminTime || sellerReadTime >= adminTime)){
            return false;
        }
    ...
    window.fetchSupportTicketBadges = async function(){
        try{
            let tickets = Array.isArray(sellerSupportTickets) ? sellerSupportTickets : [];

            if(db && sellerMail()){
                try{
                    const snap = await db.collection('seller_support_tickets')
                        .where('email','==',sellerMail())
                        .limit(250)
                        .get();

                    tickets = snap.docs.map(d => ({id:d.id, ...d.data()}));
                    sellerSupportTickets = tickets;
                }catch(e){}
            }

            const count = tickets.filter(ticketNeedsSellerRead).length;
            setBadge('Admin Support', 'badge-support-replies', count);
            return count;
        }catch(e){
            setBadge('Admin Support', 'badge-support-replies', 0);
            return 0;
        }
    };

    const oldOpenTicketDetail = window.openTicketDetail || window.viewTicketDetails;

    window.openTicketDetail = async function(id){
        try{
            const t = (sellerSupportTickets || []).find(x => String(x.id) === String(id));
            if(t){
                t.readBySeller = true;
                t.sellerRead = true;
                t.unreadForSeller = false;
                t.sellerReadAt = new Date().toISOString();

                if(db){
                    db.collection('seller_support_tickets').doc(id).set({
                        readBySeller:true,
                        sellerRead:true,
                        unreadForSeller:false,
                        sellerReadAt:new Date().toISOString()
                    }, {merge:true}).catch(()=>{});
                }
            }
        }catch(e){}

        const out = oldOpenTicketDetail ? oldOpenTicketDetail.apply(this, arguments) : undefined;

        setTimeout(() => {
            try{ fetchSupportTicketBadges(); }catch(e){}
        }, 300);

        return out;
    };

    window.viewTicketDetails = window.openTicketDetail;

    function qnaReadKey(q){
        return 'aryanta_qna_read_' + sellerMail() + '_' + [
            q.productId || '',
            q.qid || q.id || q.docId || '',
            q.question || ''
        ].join('_');
    ...
    function isQnaRead(q){
        return localStorage.getItem(qnaReadKey(q)) === '1' || q.readBySeller === true || q.sellerRead === true;
    }

    function markQnaRead(q){
        if(!q) return;
        localStorage.setItem(qnaReadKey(q), '1');
        q.readBySeller = true;
        q.sellerRead = true;

        if(q.source === 'collection' && q.docId && db){
            db.collection('product_questions').doc(q.docId).set({
                readBySeller:true,
                sellerRead:true,
                unreadForSeller:false,
                sellerReadAt:new Date().toISOString()
    ...
    function qnaUnreadCount(rows){
        return (rows || []).filter(q => !q.answer && !isQnaRead(q)).length;
    }

    function updateQnaBadges(rows){
        const count = qnaUnreadCount(rows || window.__aryQnaCache || []);
        setBadge('Customer Q&A', 'badge-qna', count);

        const stat = $('stat-pending-qna');
    ...
    window.filterQna = function(type){
        ...
        ['All','Pending','Answered'].forEach(x => {
            const el = $('qnaFilter' + x);
            if(el) el.classList.toggle('active', x === window.currentQnaFilter);
        });
    ...
    window.loadQna = async function(){
        ...
        list.innerHTML = `
            <tr class="ary-qna-card-row">
                <td colspan="4" class="ary-qna-card-cell">
                    <div class="ary-qna-empty"><i class="fas fa-spinner fa-spin"></i> Loading customer questions...</div>
                </td>
            </tr>
        `;
        ...
        try{
            if(db && sellerMail()){
                const snap = await db.collection('product_questions')
                    .where('sellerEmail','==',sellerMail())
                    .limit(250)
                    .get();
        ...
        let data = rows.filter(r => {
            const key = `${r.productId}_${r.qid}_${r.question}`;
            if(seen.has(key)) return false;
            seen.add(key);
            return !!r.question;
        }).sort((a,b) => String(b.timestamp || '').localeCompare(String(a.timestamp || '')));

        window.__aryQnaAllCache = data;
        updateQnaBadges(data);
        ...
        if(!data.length){
            list.innerHTML = `
                <tr class="ary-qna-card-row">
                    <td colspan="4" class="ary-qna-card-cell">
                        <div class="ary-qna-empty">
                            <i class="fas fa-comments"></i>
                            <h3>No ${esc(f.toLowerCase())} questions found</h3>
                        </div>
                    </td>
                </tr>
            `;
        ...
        list.innerHTML = data.map(q => {
            const answered = !!q.answer;
            const unread = !answered && !isQnaRead(q);

            return `
                <tr class="ary-qna-card-row">
                    <td colspan="4" class="ary-qna-card-cell">
                        <div class="ary-qna-mobile-card ${unread ? 'unread' : ''}">
                            <div class="ary-qna-card-head">
                                <div>
                                    <strong>${esc(q.productName || q.productId || 'Product')}</strong>
                                    <span>${esc(q.productId || '')}</span>
                                </div>
                                <em class="${answered ? 'answered' : 'pending'}">
                                    <i class="fas ${answered ? 'fa-check' : 'fa-clock'}"></i>
                                    ${answered ? 'Answered' : (unread ? 'New' : 'Needs Reply')}
                                </em>
                            </div>

                            <div class="ary-qna-question">
                                <b>Q</b>
                                <p>${esc(q.question)}</p>
                            </div>

                            <div class="ary-qna-answer">
                                <b>A</b>
                                <p>${answered ? esc(q.answer) : 'Waiting for your reply'}</p>
                            </div>

                            <button class="btn-prime w-100" onclick="openQnaModal('${js(q.productId)}','${js(q.qid)}')">
                                <i class="fas ${answered ? 'fa-pen' : 'fa-reply'}"></i>
                                ${answered ? 'Edit Reply' : 'Answer Now'}
                            </button>
                        </div>
                    </td>
                </tr>
        ...
        /* Opening Customer Q&A means visible questions are read, so badge becomes 0 */
        setTimeout(() => {
            data.forEach(markQnaRead);
            updateQnaBadges(window.__aryQnaAllCache || data);
        }, 600);
    };

    const oldOpenQnaModal = window.openQnaModal;

    window.openQnaModal = function(pId, qId){
        const q = (window.__aryQnaCache || window.__aryQnaAllCache || []).find(x =>
            String(x.productId) === String(pId) && String(x.qid) === String(qId)
        );

        if(q){
            markQnaRead(q);
            updateQnaBadges(window.__aryQnaAllCache || []);
        }

        if(oldOpenQnaModal) return oldOpenQnaModal.apply(this, arguments);
    };

    const oldSubmitQnaAnswer = window.submitQnaAnswer;

    window.submitQnaAnswer = async function(){
        const out = oldSubmitQnaAnswer ? await oldSubmitQnaAnswer.apply(this, arguments) : undefined;

        setTimeout(() => {
            try{ loadQna(); }catch(e){}
            try{ updateQnaBadges(window.__aryQnaAllCache || []); }catch(e){}
        }, 500);

        return out;
    };
})();
/* ===== B2B ITEMS: CARD ONLY IMAGE + TITLE, DETAILS IN POPUP ===== */
(function(){
    function b2bSafe(v){
    ...
    function b2bImage(p){
        return p.image || p.img || p.photo || p.productImage || "https://via.placeholder.com/360x240?text=Aryanta+B2B";
    }

    function b2bVal(p, keys, fallback){
        for(const k of keys){
    ...
    window.openB2bFullImage = function(src){
        src = src || "https://via.placeholder.com/900x900?text=Aryanta+B2B";
        const old = document.getElementById("b2bFullImageModal");
        if(old) old.remove();

        const div = document.createElement("div");
        div.id = "b2bFullImageModal";
        div.style.cssText = "position:fixed;inset:0;background:rgba(0,0,0,.92);z-index:999999;display:flex;align-items:center;justify-content:center;padding:18px;";
        div.innerHTML = `
            <button onclick="document.getElementById('b2bFullImageModal').remove()"
                style="position:absolute;top:18px;right:18px;background:#fff;color:#111;border:0;border-radius:50%;width:42px;height:42px;font-size:22px;font-weight:900;cursor:pointer;">×</button>
            <img src="${b2bSafe(src)}" style="max-width:96%;max-height:92vh;object-fit:contain;border-radius:16px;background:#fff;">
        `;
        document.body.appendChild(div);
    };
    ...
    window.loadB2bStore = function(){
        ...
        grid.classList.add("ary-b2b-grid", "ary-b2b-simple-grid");
        grid.innerHTML = `<div class="ary-b2b-empty"><i class="fas fa-spinner fa-spin"></i> Loading B2B items...</div>`;

        if(!db){
            grid.innerHTML = `<div class="ary-b2b-empty error">Firebase not ready. Please refresh page.</div>`;
            return;
        }
        ...
        db.collection("b2b_products").get().then(snap => {
            ...
            if(!b2bItems.length){
                grid.innerHTML = `<div class="ary-b2b-empty">No B2B items listed by admin yet.</div>`;
                return;
            }

            grid.innerHTML = b2bItems.map(p => {
                const img = b2bImage(p);
                const stock = Number(p.stock || p.qty || p.quantity || 0);
                const disabled = stock <= 0 ? "out" : "";

                return `
                    <div class="ary-b2b-card b2b-simple-card ${disabled}" onclick="openBuyB2bModal('${b2bJs(p.id)}')">
                        <div class="ary-b2b-img b2b-simple-img">
                            <img src="${b2bSafe(img)}" onerror="this.src='https://via.placeholder.com/360x240?text=Aryanta+B2B'">
                        </div>
                        <div class="b2b-simple-title">
                            <h4>${b2bSafe(p.name || p.title || "B2B Item")}</h4>
                        </div>
                    </div>
                `;
            }).join("");
            ...
            console.error(e);
            grid.innerHTML = `<div class="ary-b2b-empty error">Failed to load B2B items. Check Firebase rules or internet.</div>`;
        });
    };

    window.openBuyB2bModal = function(id){
        const p = (window.b2bItems || b2bItems || []).find(x => String(x.id) === String(id));
        if(!p) return;

        const stock = Number(b2bVal(p, ["stock","qty","quantity"], 0));
        if(stock <= 0) return showToast ? showToast("This B2B item is out of stock.","warning") : alert("Out of stock");

        const img = b2bImage(p);
        const moq = Math.max(1, Number(p.moq || p.minQty || p.minimumQty || 1));
        const sellingPrice = Number(b2bVal(p, ["price","sellingPrice","salePrice"], 0));
        const marketPrice = Number(b2bVal(p, ["mrp","marketPrice","regularPrice","oldPrice"], sellingPrice));
        const desc = b2bVal(p, ["description","desc","details","productDescription"], "No description added.");
        const category = b2bVal(p, ["category","type"], "Wholesale");
        const brand = b2bVal(p, ["brand","company","supplier"], "Aryanta Enterprise Admin");
        const unit = b2bVal(p, ["unit","packUnit"], "unit");
        const warranty = b2bVal(p, ["warranty","guarantee"], "Standard Admin Guarantee");
        const delivery = b2bVal(p, ["delivery","deliverySpeed"], "Dispatch & Delivery within 7 Days");

        const buyId = document.getElementById("b2bBuyId");
        const buyQty = document.getElementById("b2bBuyQty");
        const moqLabel = document.getElementById("b2bMoqLabel");
        const warrText = document.getElementById("b2bWarrText");

        if(buyId) buyId.value = id;
        if(buyQty){
            buyQty.value = moq;
            buyQty.min = moq;
            buyQty.max = stock;
        }
        if(moqLabel) moqLabel.innerText = moq;
        if(warrText) warrText.innerText = warranty;

        const info = document.getElementById("b2bProductInfo");
        if(info){
            info.innerHTML = `
                <div class="b2b-popup-product-final">
                    <img src="${b2bSafe(img)}"
                        onclick="openB2bFullImage('${b2bJs(img)}')"
                        onerror="this.src='https://via.placeholder.com/360x240?text=Aryanta+B2B'">

                    <div class="b2b-popup-info-final">
                        <span>${b2bSafe(category)}</span>
                        <h3>${b2bSafe(p.name || p.title || "B2B Item")}</h3>
                        <p>${b2bSafe(desc)}</p>
                    </div>
                </div>

                <div class="b2b-popup-detail-grid-final">
                    <div><small>Market Price</small><strong>${b2bMoney(marketPrice)}</strong></div>
                    <div><small>Selling Price</small><strong>${b2bMoney(sellingPrice)}</strong></div>
                    <div><small>Minimum Qty</small><strong>${moq}</strong></div>
                    <div><small>Available Qty</small><strong>${stock}</strong></div>
                    <div><small>Unit</small><strong>${b2bSafe(unit)}</strong></div>
                    <div><small>Brand / Supplier</small><strong>${b2bSafe(brand)}</strong></div>
                    <div><small>Warranty</small><strong>${b2bSafe(warranty)}</strong></div>
                    <div><small>Delivery</small><strong>${b2bSafe(delivery)}</strong></div>
                </div>
            `;
        }

        const shop = (activeSeller && activeSeller.shopInfo) || {};
        const addr = document.getElementById("b2bBuyAddress");
        const city = document.getElementById("b2bBuyCity");
        const state = document.getElementById("b2bBuyState");
        const pin = document.getElementById("b2bBuyPin");

        if(addr) addr.value = shop.address || activeSeller.address || "";
        if(city) city.value = shop.city || activeSeller.city || "";
        if(state) state.value = shop.state || activeSeller.state || "";
        if(pin) pin.value = shop.pincode || activeSeller.pincode || "";

        if(typeof goToB2bStep1 === "function") goToB2bStep1();
        if(typeof calcB2bTotal === "function") calcB2bTotal();

        const modal = document.getElementById("buyB2bModal");
        if(modal){
            modal.style.display = "flex";
            setTimeout(() => modal.classList.add("show"), 10);
        }
...
(function(){
if(window.__aryantaSellerNotificationRuntimeV6)return;
window.__aryantaSellerNotificationRuntimeV6=true;
const $=id=>document.getElementById(id);
const esc=v=>String(v==null?'':v).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const low=v=>String(v||'').toLowerCase().trim();
const now=()=>new Date().toISOString();
function dbx(){try{return db||window.db||firebase.firestore()}catch(e){return null}}
function seller(){try{return activeSeller||window.activeSeller||JSON.parse(localStorage.getItem('sellerToken')||'{}')}catch(e){return window.activeSeller||{}}}
function email(){return low(seller().email)}
function safeKey(v){return low(v).replace(/[^a-z0-9]/g,'_')||'seller'}
function targetOf(r){return low(r.target||r.sellerEmail||r.email||r.targetSellerEmail||'all')}
function isAll(r){const t=targetOf(r);return !t||t==='all'||t==='everyone'||t==='all sellers'||t==='all_sellers'}
function isForMe(r){const e=email();const t=targetOf(r);return isAll(r)||t===e}
function readObj(v){return v&&typeof v==='object'?v:{}}
function hasRead(type,r){const e=email(),k=safeKey(e);const rb=readObj(r.readBy||r.seenBy||r.readMap);if(rb[k]||rb[e])return true;if(Array.isArray(r.readBy)&&r.readBy.map(low).includes(e))return true;if(Array.isArray(r.seenBy)&&r.seenBy.map(x=>low(x.email||x)).includes(e))return true;const s=seller();const map=type==='popup'?readObj(s.popupReadMap||s.sellerPopupReadMap):readObj(s.notificationReadMap||s.notifReadMap||s.readNotifications);if(map[r.id]||map[r.readKey]||map[String(r.id)])return true;if(type==='popup'&&!isAll(r)&&r.isRead===true)return true;if(type==='notification'&&!isAll(r)&&(r.isRead===true||r.read===true))return true;return false}
function showToastSafe(m,t='info'){try{(window.showToast||window.toast||alert)(m,t)}catch(e){alert(m)}}
function normalizeLink(v){v=String(v||'').trim();if(!v)return'';return /^https?:\/\//i.test(v)?v:'https://'+v}
async function setSellerReadMap(type,id){const d=dbx(),e=email();if(!d||!e||!id)return;const s=seller();const key=type==='popup'?'popupReadMap':'notificationReadMap';const map=Object.assign({},readObj(s[key]));map[id]=true;try{if(typeof activeSeller!=='undefined'&&activeSeller)activeSeller[key]=map;window.activeSeller=Object.assign({},window.activeSeller||{}, {[key]:map});localStorage.setItem('sellerToken',JSON.stringify(Object.assign({},s,{[key]:map})))}catch(x){}try{await d.collection('sellers').doc(e).set({[key]:map,updatedAt:now()},{merge:true})}catch(err){}}
async function markRead(collection,id,type,target){const d=dbx(),e=email();if(!d||!e||!id)return;await setSellerReadMap(type,id);const k=safeKey(e);const patch={updatedAt:now()};patch['readBy.'+k]={email:e,readAt:now()};patch['seenBy.'+k]=true;if(type==='popup'&&target!=='all')patch.isRead=true;try{await d.collection(collection).doc(id).set(patch,{merge:true})}catch(err){}}
function asNotification(doc){const r=doc.data?doc.data():doc;const id=doc.id||r.id;if(/popup/i.test(String(r.type||r.channel||''))||r.isPopup===true)return null;if(!isForMe(r))return null;return {id,readKey:id,sourceCollection:'admin_broadcasts',title:r.title||r.subject||'Aryanta Notice',text:r.message||r.text||r.body||r.title||'New notice from Aryanta',message:r.message||r.text||r.body||'',time:r.timestamp||r.createdAt||r.time||now(),link:r.link||r.url||r.actionLink||r.buttonLink||'',buttonText:r.buttonText||r.actionText||'',read:hasRead('notification',{id,...r,readKey:id}),raw:r}}
function renderNotifLists(rows){const badge=$('notifBadge');const unread=rows.filter(n=>!n.read).length;if(badge){if(unread>0){badge.innerText=unread;badge.style.display='block'}else badge.style.display='none'}const full=$('fullNotifList');if(full){full.innerHTML=rows.length?rows.map(n=>`<div style="padding:15px;border:1px solid var(--border-color);background:var(--surface-2);border-radius:12px;margin-bottom:10px;cursor:pointer;" onclick="openFullNotif('${esc(n.id)}')"><div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;"><span style="width:10px;height:10px;border-radius:50%;background:${n.read?'var(--success)':'var(--danger)'};display:inline-block;"></span><strong style="font-size:15px;color:var(--text-main);">${esc(n.title||'Aryanta Notice')}</strong></div><div style="font-size:13px;color:var(--text-light);font-weight:700;line-height:1.5;">${esc(n.text||'')}</div><div style="font-size:11px;color:var(--text-light);margin-top:8px;"><i class="fas fa-clock"></i> ${esc(new Date(n.time||Date.now()).toLocaleString('en-IN'))}${n.link?' • Link attached':''}</div></div>`).join(''):'<div style="text-align:center;padding:24px;color:var(--text-light);font-weight:800;"><i class="fas fa-bell-slash" style="font-size:28px;margin-bottom:10px;display:block;"></i>No notifications.</div>'}const small=$('notifList');if(small){small.innerHTML=rows.length?rows.slice(0,5).map(n=>`<div class="suggestion-item" onclick="openFullNotif('${esc(n.id)}'); if(window.toggleNotifications)toggleNotifications();"><strong>${n.read?'✅':'🔴'} ${esc(n.title||'Aryanta Notice')}</strong><span>${esc((n.text||'').slice(0,80))}</span></div>`).join(''):'<div style="text-align:center; padding:30px; color:var(--text-light); font-size:14px;"><i class="fas fa-box-open" style="font-size:30px; margin-bottom:10px;"></i><br>No new messages.</div>'}}
window.fetchNotifications=async function(){const d=dbx(),e=email();if(!d||!e)return;let rows=[];try{let snap;try{snap=await d.collection('admin_broadcasts').orderBy('timestamp','desc').limit(80).get()}catch(x){snap=await d.collection('admin_broadcasts').limit(80).get()}snap.forEach(doc=>{const n=asNotification(doc);if(n)rows.push(n)})}catch(err){}try{let snap=await d.collection('seller_notifications').where('sellerEmail','==',e).limit(40).get();snap.forEach(doc=>{const r=doc.data();const n={id:doc.id,readKey:doc.id,sourceCollection:'seller_notifications',title:r.title||r.subject||'Aryanta Notice',text:r.message||r.text||r.body||'',message:r.message||r.text||r.body||'',time:r.timestamp||r.createdAt||r.time||now(),link:r.link||r.url||r.actionLink||r.buttonLink||'',buttonText:r.buttonText||r.actionText||'',read:hasRead('notification',{id:doc.id,...r,readKey:doc.id}),raw:r};rows.push(n)})}catch(e2){}const seen=new Set();rows=rows.filter(n=>{const k=(n.sourceCollection||'')+'_'+n.id;if(seen.has(k))return false;seen.add(k);return true}).sort((a,b)=>new Date(b.time||0)-new Date(a.time||0));try{adminNotifications=rows;window.adminNotifications=rows;unreadNotifCount=rows.filter(n=>!n.read).length}catch(e3){window.adminNotifications=rows}renderNotifLists(rows)};
window.renderNotificationsAryanta=function(){let rows=[];try{rows=Array.isArray(adminNotifications)?adminNotifications:[]}catch(e){rows=window.adminNotifications||[]}renderNotifLists(rows)};
window.openFullNotif=window.openFullNotifFinal=window.aryOpenNotification=async function(id){let rows=[];try{rows=Array.isArray(adminNotifications)?adminNotifications:[]}catch(e){rows=window.adminNotifications||[]}const n=rows.find(x=>String(x.id)===String(id)||String(x.readKey)===String(id));if(!n)return;const link=normalizeLink(n.link);const cont=$('notifDetailContent');const mod=$('notificationDetailModal');if(cont)cont.innerHTML=`<div style="background:var(--surface-2);padding:20px;border-radius:16px;border:1px solid var(--border-color);"><div style="display:flex;align-items:center;gap:12px;margin-bottom:14px;"><div style="width:46px;height:46px;border-radius:14px;background:var(--primary);color:white;display:flex;align-items:center;justify-content:center;"><i class="fas fa-bell"></i></div><div><h3 style="margin:0;font-weight:900;color:var(--text-main);">${esc(n.title||'Aryanta Notice')}</h3><small style="color:var(--text-light);font-weight:800;">${esc(new Date(n.time||Date.now()).toLocaleString('en-IN'))}</small></div></div><div style="font-size:15px;line-height:1.7;font-weight:700;color:var(--text-main);white-space:pre-wrap;">${esc(n.message||n.text||'')}</div>${link?`<a href="${esc(link)}" target="_blank" rel="noopener" class="btn-prime w-100" style="text-decoration:none;margin-top:18px;"><i class="fas fa-external-link-alt"></i> ${esc(n.buttonText||'Open Attached Link')}</a>`:''}</div>`;if(mod){mod.style.display='flex';setTimeout(()=>mod.classList.add('show'),10)}else if(link)window.open(link,'_blank','noopener');n.read=true;await markRead(n.sourceCollection||'admin_broadcasts',n.id,'notification',targetOf(n.raw||{}));renderNotifLists(rows)};
function popupCandidate(doc){const r=doc.data?doc.data():doc;const id=doc.id||r.id;if(r.active===false||r.deleted===true)return null;if(!isForMe(r))return null;if(hasRead('popup',{id,...r,readKey:id}))return null;return {id,sourceCollection:'seller_popups',title:r.title||r.subject||'Aryanta Notice',message:r.message||r.body||r.text||'',time:r.timestamp||r.createdAt||r.time||now(),link:r.link||r.url||r.actionLink||r.buttonLink||'',buttonText:r.buttonText||r.actionText||'Open',target:targetOf(r),raw:r}}
window.checkAdminPopups=async function(){const d=dbx(),e=email();if(!d||!e)return;let rows=[];try{const a=await d.collection('seller_popups').where('sellerEmail','==',e).limit(20).get();a.forEach(doc=>{const p=popupCandidate(doc);if(p)rows.push(p)})}catch(err){}try{const b=await d.collection('seller_popups').where('target','==','all').limit(20).get();b.forEach(doc=>{const p=popupCandidate(doc);if(p)rows.push(p)})}catch(err2){}try{const c=await d.collection('seller_popups').where('sellerEmail','==','all').limit(20).get();c.forEach(doc=>{const p=popupCandidate(doc);if(p)rows.push(p)})}catch(err3){}const seen=new Set();rows=rows.filter(p=>{if(seen.has(p.id))return false;seen.add(p.id);return true}).sort((a,b)=>new Date(a.time||0)-new Date(b.time||0));const p=rows[0];if(!p)return;const title=$('adminPopupTitle'),msg=$('adminPopupMessage'),modal=$('adminPopupModal');if(title)title.innerText=p.title||'Aryanta Notice';if(msg){const link=normalizeLink(p.link);msg.innerHTML=`<div style="white-space:pre-wrap;">${esc(p.message||'')}</div>${link?`<a href="${esc(link)}" target="_blank" rel="noopener" class="btn-prime w-100" style="text-decoration:none;margin-top:18px;"><i class="fas fa-external-link-alt"></i> ${esc(p.buttonText||'Open Link')}</a>`:''}`;}if(modal){modal.style.display='flex';setTimeout(()=>modal.classList.add('show'),10)}await markRead('seller_popups',p.id,'popup',p.target)};
document.addEventListener('DOMContentLoaded',()=>{setTimeout(()=>{try{window.fetchNotifications&&window.fetchNotifications();window.checkAdminPopups&&window.checkAdminPopups()}catch(e){}},1200)});
})();
