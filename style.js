...
/* MODIFIED GRID TO FIT 5 ITEMS */
.att-stats-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(110px, 1fr)); gap: 15px; }
...
.slip-notice-card:hover { transform: translateY(5px); }
...
/* --- FIX: MESSAGE POPUP TEXT VISIBILITY --- */
#msg-popup-content {
    background: #f8fafc;
    padding: 20px;
    border-radius: 12px;
    border: 1px solid #e2e8f0;
    color: #1e293b !important; /* Forces dark text */
    text-align: left;
}
...
/* --- FIX: ATTENDANCE BUTTON VISIBILITY --- */
/* Makes the active button pop out more visually */
.st-btn.active {
...
/* Ensure the inactive buttons look "off" */
.st-btn:not(.active) {
    opacity: 0.6;
    background: #f1f5f9;
    border: 1px solid #cbd5e1;
}
.st-btn:not(.active):hover {
    opacity: 1;
    background: #e2e8f0;
}
/* ... (All previous styles) ... */
